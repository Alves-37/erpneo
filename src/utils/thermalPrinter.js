...
class ThermalPrinter {
  constructor() {
    ...
    this.paperWidth = 80; // 80mm formato padrão (seu papel)
    this.paperHeight = 297; // 297mm altura máxima
  ...
  // 📄 Gerar HTML para impressão Web (Formato Otimizado para 80mm x 297mm)
      ...
      <style>
        @media print {
          ...
          body { 
            ...
            font-size: 9px;
            width: 74mm; /* 80mm - margens */
            line-height: 1.1;
            max-height: 291mm; /* 297mm - margens */
            overflow: hidden;
          }
          ...
          .total { font-weight: bold; font-size: 10px; }
          ...
          .optimize-height { height: 280mm; }
        }
      </style>
    </head>
    <body>
      <div class="receipt optimize-height">
        <!-- Cabeçalho Otimizado -->
        <div class="center">
          <div class="company-name">${company.name}</div>
          ${company.phone ? `<div class="tiny">${company.phone}</div>` : ''}
          ${company.address ? `<div class="tiny">${company.address}</div>` : ''}
        </div>
        ...
        <div class="center">
          <div class="title">RECIBO/FATURA</div>
          <div class="small">#${sale.id} ${new Date(sale.created_at).toLocaleDateString('pt-BR')} ${new Date(sale.created_at).toLocaleTimeString('pt-BR', {hour: '2-digit', minute:'2-digit'})}</div>
        </div>
        
        ${customer ? `
        <div class="border-bottom"></div>
        <div class="center compact">
          <div class="small">Cliente: ${customer.name}</div>
          ${customer.nuit ? `<div class="tiny">NUIT: ${customer.nuit}</div>` : ''}
        </div>
        ` : ''}
        ...
        <div class="left-align">
          ${items.map(item => `
            <div class="item compact">
              <span class="small">${item.quantity}x ${item.name.length > 20 ? item.name.substring(0, 20) + '...' : item.name}</span>
        ...
        <div>
          ...
          ${sale.paid && sale.paid !== sale.total ? `
          <div class="item">
            <span>PAGO:</span>
            <span>${sale.paid.toFixed(2)} MT</span>
          </div>
          ` : ''}
          ${sale.change ? `
          <div class="item">
        ...
        <div class="center">
          ...
          <div class="tiny">${new Date().toLocaleDateString('pt-BR')} ${new Date().toLocaleTimeString('pt-BR', {hour: '2-digit', minute:'2-digit'})}</div>
          ${company.nuit ? `<div class="tiny">NUIT: ${company.nuit}</div>` : ''}
        </div>
      </div>
  ...
  // 🔌 Gerar comandos ESC/POS (Formato Otimizado para 80mm x 297mm)
  generateESCPOSCommands(saleData) {
    ...
    // Definir área de impressão (limitar a 80mm)
    commands.push(Buffer.from([0x1D, 0x28, 0x43])); // GS (C - Definir área de impressão
    
    // Fonte média para 80mm (legível mas compacta)
    commands.push(Buffer.from([0x1B, 0x21, 0x00])); // ESC ! 0 - Fonte padrão

    // Centralizar
    commands.push(Buffer.from([0x1B, 0x61, 0x01])); // ESC a 1 - Center align

    // Cabeçalho Otimizado para 80mm
    ...
    // Linha separadora para 80mm
    commands.push(Buffer.from('----------------------------------------\n'));

    // Título Otimizado
    commands.push(Buffer.from([0x1B, 0x45, 0x01])); // ESC E 1 - Bold on
    commands.push(Buffer.from('RECIBO/FATURA\n'));
    commands.push(Buffer.from([0x1B, 0x45, 0x00])); // ESC E 0 - Bold off
    
    commands.push(Buffer.from(`#${sale.id} ${new Date(sale.created_at).toLocaleDateString('pt-BR')} ${new Date(sale.created_at).toLocaleTimeString('pt-BR', {hour: '2-digit', minute:'2-digit'})}\n`));
    ...
    if (customer) {
      ...
      if (customer.nuit) {
        commands.push(Buffer.from(`NUIT: ${customer.nuit}\n`));
      }
      commands.push(Buffer.from([0x1B, 0x21, 0x00])); // ESC ! 0 - Fonte padrão
    }
    ...
    // Itens otimizados para 80mm (máximo 42 caracteres por linha)
    items.forEach(item => {
      ...
      // Ajustar para 42 caracteres totais
      const availableSpace = 42 - itemLine.length;
    ...
    if (sale.paid && sale.paid !== sale.total) {
      commands.push(Buffer.from(`PAGO: ${sale.paid.toFixed(2)} MT\n`));
    }
    if (sale.change) {
      commands.push(Buffer.from(`TROCO: ${sale.change.toFixed(2)} MT\n`));
    }
    ...
    commands.push(Buffer.from(new Date().toLocaleDateString('pt-BR') + ' ' + new Date().toLocaleTimeString('pt-BR', {hour: '2-digit', minute:'2-digit'}) + '\n'));
    
    if (company.nuit) {
      commands.push(Buffer.from(`NUIT: ${company.nuit}\n`));
    }

    // FORÇAR PARADA DE AVANÇO DE PAPEL (CRÍTICO para 80mm)
    commands.push(Buffer.from([0x1B, 0x4A, 0x00])); // ESC J 0 - Sem avanço de linha
    commands.push(Buffer.from([0x1B, 0x64, 0x00])); // ESC d 0 - Sem avanço de linha
    commands.push(Buffer.from([0x1B, 0x64, 0x00])); // ESC d 0 - Repetir para garantir
    
    // Limpar buffer e resetar
    commands.push(Buffer.from([0x1B, 0x40])); // ESC @ - Reset da impressora
    
    // Corte parcial (em vez de corte total - economiza papel)
    commands.push(Buffer.from([0x1D, 0x56, 0x01])); // GS V 1 - Partial cut
  ...
  // 🍳 Gerar HTML para Ticket de Cozinha (Otimizado para 80mm)
    ...
    <head>
      ...
      <style>
        @media print {
          ...
          body { font-family: 'Courier New', monospace; font-size: 11px; width: 74mm; line-height: 1.2; }
          .center { text-align: center; }
          .bold { font-weight: bold; }
          .border { border-top: 1px dashed #000; margin: 3px 0; }
          .item { font-size: 12px; margin: 3px 0; }
          .kitchen-title { font-size: 16px; font-weight: bold; border: 2px solid #000; padding: 3px; margin-bottom: 5px; }
        }
      </style>
    </head>
    <body>
      <div class="center">
        ...
        <div class="bold" style="font-size: 18px; margin: 5px 0;">
          MESA: ${order.table_number}<br>
          CLIENTE: ${order.seat_number}
        </div>
        <div style="font-size: 14px;">${new Date().toLocaleTimeString('pt-BR', {hour: '2-digit', minute:'2-digit'})}</div>
      </div>
      ...
      ${items.map(item => `
        <div class="item">
          <span class="bold" style="font-size: 14px;">${item.quantity || item.qty}x</span> 
          <span style="font-size: 13px;">${item.name || item.product_name || `Item #${item.product_id}`}</span>
          ${item.notes ? `<br><span style="font-size: 10px; font-style: italic;">OBS: ${item.notes}</span>` : ''}
        </div>
      `).join('')}
      
      <div class="border"></div>
      
      <div class="center bold" style="font-size: 14px;">BOM TRABALHO!</div>
  ...
  async configurePrinter(config) {
    this.paperWidth = config.paperWidth || 80; // 80mm padrão (seu papel)
    this.paperHeight = config.paperHeight || 297; // 297mm altura
    ...
    return { 
      success: true, 
      message: `Impressora ${this.printerModel} configurada (${this.paperWidth}mm x ${this.paperHeight}mm)!` 
    };
  }
