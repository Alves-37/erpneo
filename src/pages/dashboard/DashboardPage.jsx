...
export default function DashboardPage() {
  ...
  const [expiryModalLoading, setExpiryModalLoading] = useState(false)
  ...
  async function openExpiryModal() {
    ...
    setExpiryModalLoading(true)
    try {
      const estId = isAdmin ? (establishment?.id || undefined) : undefined
      const all = await getDashboardExpiryAlerts({ days: 30, limit: 200, establishment_id: estId })
      setExpiryModalAlerts(all)
      _clearExpiryAlertsDisable()
    } catch {
      setExpiryModalAlerts(null)
    } finally {
      setExpiryModalLoading(false)
    }
  }
    ...
    <div>
              ...
              <div className="rounded-2xl border border-slate-800 bg-slate-900 p-3 sm:p-4">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div className="inline-flex items-center rounded-xl border border-amber-900/60 bg-amber-950/25 px-3 py-1 text-xs font-semibold text-amber-200">
                      {loading || !expiryAlerts ? '-' : expiryAlerts?.expiring_soon_count ?? 0}
                    </div>
                    <button
                      type="button"
                      onClick={openExpiryModal}
                      className="ml-1 inline-flex items-center rounded-xl border border-slate-800 bg-slate-950 px-3 py-1 text-xs font-semibold text-slate-200 hover:bg-slate-900"
                    >
                      Ver todos
                    </button>
                  </div>
                ...
                <div className="mt-3 rounded-xl border border-slate-800 bg-slate-950">
                  {!expiryAlerts && !loading ? (
                    ...
                    const it = (expiryAlerts?.items || [])[0]
                    const daysToExpire = Number(it?.days_to_expire || 0)
                    const isExpired = daysToExpire < 0
                    return (
                      <div className="flex items-center justify-between gap-3 px-3 py-3">
                        <div className="min-w-0">
                          <div className="text-xs font-semibold text-slate-100 truncate" title={it?.name || ''}>
                            {it?.name}
                          </div>
                          <div className="mt-0.5 text-[11px] text-slate-400">Validade: {it?.validade || '-'}</div>
                        </div>
                        <div
                          className={`shrink-0 inline-flex items-center rounded-xl border px-3 py-1 text-[11px] font-semibold ${
                            isExpired
                        ...
                        >
                          {isExpired ? `${Math.abs(daysToExpire)} dias atrasado` : `${daysToExpire} dias`}
                        </div>
                      </div>
                    )
                  })() : (
      ...
      >
        <div className="flex items-start justify-between gap-4">
          <div>
            <div className="text-sm font-semibold text-white">Produtos com validade</div>
            <div className="mt-1 text-xs text-slate-400">Próximos {expiryModalAlerts?.days_window ?? expiryAlerts?.days_window ?? 30} dias</div>
          </div>
          <div className="flex items-center gap-2">
            <div className="inline-flex items-center rounded-xl border border-rose-900/60 bg-rose-950/30 px-3 py-1 text-xs font-semibold text-rose-200">
              Expirados: {expiryModalLoading || !expiryModalAlerts ? '-' : expiryModalAlerts?.expired_count ?? 0}
            </div>
            <div className="inline-flex items-center rounded-xl border border-amber-900/60 bg-amber-950/25 px-3 py-1 text-xs font-semibold text-amber-200">
              A expirar: {expiryModalLoading || !expiryModalAlerts ? '-' : expiryModalAlerts?.expiring_soon_count ?? 0}
            </div>
          </div>
        </div>
        ...
        <div className="mt-4 rounded-xl border border-slate-800 bg-slate-950">
          {expiryModalLoading ? (
            <div className="px-4 py-4 text-sm text-slate-300">Carregando...</div>
          ) : (expiryModalAlerts?.items || []).length ? (
            <div className="divide-y divide-slate-800">
              {(expiryModalAlerts?.items || []).map((it) => {
                const daysToExpire = Number(it?.days_to_expire || 0)
                const isExpired = daysToExpire < 0
                const categoryLabel = isExpired ? 'Expirado' : 'A expirar'
                const categoryClass = isExpired
                  ? 'border-rose-900/60 bg-rose-950/30 text-rose-200'
                  : 'border-amber-900/60 bg-amber-950/25 text-amber-200'
                return (
                  <div key={it.product_id} className="flex items-center justify-between gap-4 px-4 py-3">
                    <div className="min-w-0">
                      <div className="text-sm font-semibold text-slate-100 truncate" title={it.name || ''}>{it.name}</div>
                      <div className="mt-0.5 text-xs text-slate-400">Validade: {it.validade}</div>
                    </div>
                    <div className="flex items-center gap-2">
                      <div className={`inline-flex items-center rounded-xl border px-3 py-1 text-xs font-semibold ${categoryClass}`}>{categoryLabel}</div>
                      <div className={`inline-flex items-center rounded-xl border px-3 py-1 text-xs font-semibold ${categoryClass}`}>
                        {isExpired ? `${Math.abs(daysToExpire)} dias atrasado` : `${daysToExpire} dias`}
                      </div>
                    </div>
                  </div>
            ...
            <div className="px-4 py-4 text-sm text-slate-300">Sem alertas de validade.</div>
          )}
        </div>
      </Modal>
    </div>
