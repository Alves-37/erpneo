      ...
      <div className="mt-6 rounded-2xl border border-slate-800 bg-slate-900 p-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
          <label className="grid gap-1.5">
            <div className="text-[10px] font-bold uppercase tracking-wider text-slate-500">Produto</div>
            <select
              value={productId}
          ...
          <label className="grid gap-1.5">
            <div className="text-[10px] font-bold uppercase tracking-wider text-slate-500">Local</div>
            <select
              value={locationId}
          ...
          <label className="grid gap-1.5">
            <div className="text-[10px] font-bold uppercase tracking-wider text-slate-500">Tipo</div>
            <select
              value={movementType}
          ...
          <label className="grid gap-1.5">
            <div className="text-[10px] font-bold uppercase tracking-wider text-slate-500">De</div>
            <input
              value={dateFrom}
              onChange={(e) => setDateFrom(e.target.value)}
              className="w-full rounded-xl border border-slate-800 bg-slate-950 px-3 py-2 text-sm text-slate-100 focus:outline-none focus:ring-2 focus:ring-brand-600 appearance-none min-w-0 overflow-hidden"
              type="datetime-local"
            />
          </label>

          <label className="grid gap-1.5">
            <div className="text-[10px] font-bold uppercase tracking-wider text-slate-500">Até</div>
            <input
              value={dateTo}
        ...
        <div className="flex flex-col sm:flex-row gap-2 mt-4 pt-4 border-t border-slate-800/50">
          <button
            type="button"
            onClick={() => loadHistory()}
            className="flex-1 rounded-xl bg-brand-600 hover:bg-brand-700 px-6 py-2.5 text-sm font-bold text-white shadow-lg shadow-brand-900/20 transition-all active:scale-95"
          >
            Filtrar
          </button>
          <button
            ...
            className="flex-1 rounded-xl border border-slate-800 bg-slate-950 hover:bg-slate-800 px-6 py-2.5 text-sm font-bold text-slate-100 transition-all active:scale-95"
          >
            Limpar
          </button>
        </div>
      ...
      <div className="mt-6 overflow-hidden rounded-2xl border border-slate-800 bg-slate-900">
        <div className="border-b border-slate-800 px-4 py-3">
          ...
          <div className="mt-3 md:hidden">
            {/* Removido seletor de colunas, agora usamos lista de cards */}
          </div>
        </div>
        ...
        <div className="md:hidden">
          {loading ? (
            <div className="px-4 py-12 flex flex-col items-center justify-center gap-3">
              ...
              <div className="text-sm text-slate-400">Carregando movimentos...</div>
            </div>
          ) : items.length ? (
            <div className="divide-y divide-slate-800">
              {items.map((m) => {
                ...
                return (
                  <div key={m.id} className="p-4 bg-slate-950/20 active:bg-slate-900 transition-colors">
                    <div className="flex items-start justify-between gap-3 mb-2">
                      <div className="min-w-0 flex-1">
                        <div className="text-[10px] font-bold uppercase tracking-wider text-slate-500 mb-0.5">
                          {formatDateTime(m.created_at)}
                        </div>
                        <div className="text-sm font-bold text-white leading-tight mb-1" title={p?.name || ''}>
                          {p?.name || `#${m.product_id}`}
                        </div>
                      </div>
                      <div className="shrink-0">
                        <span
                          className={`inline-flex items-center rounded-lg border px-2.5 py-1 text-xs font-black tabular-nums shadow-sm ${qtyColor(m.qty_delta)}`}
                        >
                    ...
                    <div className="flex flex-wrap gap-1.5 mt-2">
                      <span
                        className={`inline-flex items-center rounded-md border px-2 py-0.5 text-[10px] font-bold uppercase ${movementTypeColor(m.movement_type)}`}
                      >
                        {movementTypeLabel(m.movement_type)}
                      </span>
                      <span
                        className={`inline-flex items-center rounded-md border px-2 py-0.5 text-[10px] font-bold uppercase ${referenceColor(m.reference_type)}`}
                      >
                        {refText}
                      </span>
                      <span className="inline-flex items-center rounded-md border border-slate-800 bg-slate-900 px-2 py-0.5 text-[10px] font-medium text-slate-400">
                        📍 {l ? l.name : `#${m.location_id}`}
                      </span>
                    </div>
                    
                    {m.notes ? (
                      <div className="mt-3 rounded-lg bg-slate-900/80 p-2.5 text-[11px] text-slate-400 italic border-l-2 border-slate-700 shadow-inner">
                        {m.notes}
                      </div>
                    ) : null}
                  </div>
            ...
            <div className="px-4 py-12 text-center text-sm text-slate-500">Sem movimentos registados.</div>
          )}
        </div>
